...
pub fn group(_attrs: TokenStream1, input: TokenStream1) -> TokenStream1 {
    ...
    let afts = Afts::from_iter(pairs.iter().a().map(|req| match &req.aft {
        Some(aft) => (req.persist, aft.clone()),
    ...
    let any_cached_aft_short_e = if afts.cached_e.is_empty() {
        afts.e.clone()
    } else {
        quote! { , 'e }
    };

    let all_afts = TokenStream::from_iter(
        afts.by_tyn
            .values()
            .flat_map(|(persist, name, ty)| {
                if !persist {
            ...
            .chain(std::iter::once_with(|| {
                quote! { Cache(CachedAft<P #any_cached_aft_short_e>), }
            })),
    );

    let all_cached_afts =
        TokenStream::from_iter(afts.by_tyn.values().filter(|(persist, _, _)| *persist).map(
            |(_, name, ty)| {
                let name = Ident::new(name, Span::call_site());
                quote! { #name(#ty), }
            },
    ...
    let impl_from_cached_aft = quote! {
        impl<P: Pf> From<CachedAft<P #any_cached_aft_short_e>> for Aft<P #any_aft_short_e> {
            fn from(cached: CachedAft<P #any_cached_aft_short_e>) -> Self {
                Self::Cache(cached)
            }
        }
    };

    let aft_wrap = TokenStream::from_iter(afts.by_tyn.values().map(|(persist, name, ty)| {
        let name = Ident::new(name, Span::call_site());

        let wrapped = if *persist {
            quote! { Self::Cache(CachedAft::#name(value)) }
        } else {
    ...
    let impl_cache_aft = quote! {
        impl<'e, P: Pf> Cache<'e, P> for Aft<P #any_aft_short_e> {
            fn cache(&self) -> Option<&P::CachedAft<'e>> {
                match self {
                    Self::Cache(cached) => Some(UnLike::un_like_ref(cached)),
                    _ => None
                }
            }
        }
    ...
    let aft_unwrap = TokenStream::from_iter(pairs.iter().a().map(|req| {
        ...
        let aft_ty_in_a = parse::<Type>(
            TokenStream1::from_str(&tyn.replace("'e", "'a")).expect("failed to rename"),
        )
        .expect("aft_ty_in_a");

        let prong = if req.persist {
            quote! { Aft::Cache(CachedAft::#aft_ident(aft)) }
        } else {
            quote! { Aft::#aft_ident(aft) }
        };
    ...
    quote! {
        mod requests {
            ...
            pub enum CachedAft<P: Pf #any_aft_short_e> {
                #all_cached_afts
            }
            ...
            impl Pf for PfIn {
                ...
                type CachedAft<'e> = CachedAft<Self #any_aft_short_e>;
