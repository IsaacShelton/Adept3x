...
impl<'a, T: Inflow<LexedLine>> Parser<'a, T> {
    ...
    pub fn parse_group(&mut self) -> Result<Group, PreprocessorError> {
        ...
        while let PreTokenLine::Line(tokens, _) = self.lines.peek().as_ok_ref()? {
    ...
    pub fn parse_group_part(&mut self) -> Result<GroupPart, PreprocessorError> {
        ...
        match directive_name {
            Some(directive_name @ ("if" | "ifdef" | "ifndef")) => {
                ...
                while let PreTokenLine::Line(peek_line, _) = self.lines.peek().as_ok_ref()? {
                ...
                let else_group = match next_line.as_ref() {
                    Ok(line) => match line {
                        PreTokenLine::Line(pre_tokens, _) => {
                            if let Some("else") = peek_directive_name(pre_tokens) {
                ...
                match self.lines.peek().as_ok_ref()? {
    ...
    match first_tokens::<2>(line) {
        Some(
            [
                ...
                PreTokenKind::Identifier(directive_name),
            ],
        ) => Some(directive_name),
        _ => None,
    }
