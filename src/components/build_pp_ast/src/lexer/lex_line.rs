...
use crate::error::{PreprocessorError, PreprocessorErrorKind};
use pp_token::{Encoding, PreToken, PreTokenKind, Punctuator};
use source_files::Source;
use text::{Character, Text, is_c_non_digit};
        ...
        match &mut state {
            State::Idle => {
                ...
                match c {
                    ...
                    '(' => push_punctuator_token(&mut tokens, OpenParen { preceeded_by_whitespace: preceded_by_whitespace }, source),
                ...
                preceded_by_whitespace = match c {
                    ' ' | '\t' | /*\v*/ '\u{0B}' | /*\f*/ '\u{0C}' => true,
                    _ => false,
                };
            }
