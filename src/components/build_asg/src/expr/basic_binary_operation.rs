...
pub fn resolve_basic_binary_operator(
    ...
    let resolved_operator = match ast_operator {
        ast::BasicBinaryOperator::Add => ty.numeric_mode().map(asg::BasicBinaryOperator::Add),
        ast::BasicBinaryOperator::Subtract => {
            ty.numeric_mode().map(asg::BasicBinaryOperator::Subtract)
        }
        ast::BasicBinaryOperator::Multiply => {
            ty.numeric_mode().map(asg::BasicBinaryOperator::Multiply)
        }
