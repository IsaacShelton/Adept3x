use super::{ASTRecordLayout, record_info::RecordInfo};
use crate::{ir, target_layout::type_layout_cache::TypeLayoutCache};
use data_units::{BitUnits, ByteUnits};
use diagnostics::{Diagnostics, WarningDiagnostic};
use target::TargetOsExt;

#[derive(Debug)]
pub struct ItaniumRecordLayoutBuilder<'a, 'env> {
...
impl<'a, 'env: 'a> ItaniumRecordLayoutBuilder<'a, 'env> {
    pub fn generate(
    ...
    pub fn layout(&mut self, record: &'a RecordInfo<'env>) {
    ...
    pub fn layout_fields(&mut self, record: &RecordInfo<'env>) {
    ...
    pub fn layout_field(
        ...
        let field_class = field.as_cxx_record_field();
        ...
        if field.ir_type.is_incomplete_array() {
            ...
            if is_potentially_overlapping(field) {
                todo!(
                    "ItaniumRecordLayoutBuilder::layout_field for is_potentially_overlapping is not fully implemented yet"
                );
...
/// Keeps track of the offsets of different empty sub-objects for C++ (but not C) record layouts
#[derive(Debug)]
