...
pub fn size_of<'env>(ir_type: &ir::Type<'env>, ir_module: &ir::Ir<'env>) -> ByteUnits {
    match ir_type {
        ir::Type::Ptr(_) => ByteUnits::of(8),
        ir::Type::Bool => ByteUnits::of(1),
        ir::Type::I(int_bits, _) => int_bits.bytes(),
        ir::Type::F(float_bits) => float_bits.bytes(),
        ir::Type::Void => ByteUnits::of(0),
        ...
        ir::Type::Struct(struct_ref) => {
            ...
            structure
                .fields
                .iter()
                .fold(ByteUnits::of(0), |acc, field| {
                    acc + size_of(&field.ir_type, ir_module)
                })
        }
        ...
        ir::Type::IncompleteArray(_) => ByteUnits::of(8),
    }
}
