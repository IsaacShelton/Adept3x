...
use crate::ir::{OverflowOperation, OverflowOperator};
use llvm_sys::{
    core::{
        LLVMAddFunction, LLVMAppendBasicBlock, LLVMBuildCall2, LLVMBuildUnreachable,
        LLVMFunctionType, LLVMGetNamedFunction, LLVMInt1Type, LLVMInt8Type, LLVMInt16Type,
        LLVMInt32Type, LLVMInt64Type, LLVMPointerType, LLVMStructType, LLVMVoidType,
    },
...
pub struct Intrinsics {
    ...
    _memset: OnceCell<(LLVMValueRef, LLVMTypeRef)>,
    stacksave: OnceCell<(LLVMValueRef, LLVMTypeRef)>,
    stackrestore: OnceCell<(LLVMValueRef, LLVMTypeRef)>,
    _va_start: OnceCell<(LLVMValueRef, LLVMTypeRef)>,
    _va_end: OnceCell<(LLVMValueRef, LLVMTypeRef)>,
    _va_copy: OnceCell<(LLVMValueRef, LLVMTypeRef)>,
...
impl Intrinsics {
    pub unsafe fn new(module: &BackendModule) -> Self {
        Self {
            module: module.get(),
            memcpy: OnceCell::new(),
            _memset: OnceCell::new(),
            stacksave: OnceCell::new(),
            stackrestore: OnceCell::new(),
            _va_start: OnceCell::new(),
            _va_end: OnceCell::new(),
            _va_copy: OnceCell::new(),
    ...
    pub unsafe fn memcpy(&mut self) -> (LLVMValueRef, LLVMTypeRef) {
        *self.memcpy.get_or_init(|| {
            ...
            (
                LLVMAddFunction(
                    self.module,
                    c"llvm.memcpy.p0i8.p0i8.i64".as_ptr(),
                    signature,
                ),
                signature,
            )
    ...
    pub fn stacksave(&self) -> (LLVMValueRef, LLVMTypeRef) {
        *self.stacksave.get_or_init(|| unsafe {
            ...
            (
                LLVMAddFunction(self.module, c"llvm.stacksave.p0".as_ptr(), signature),
                signature,
            )
        })
    }
    ...
    pub fn stackrestore(&self) -> (LLVMValueRef, LLVMTypeRef) {
        *self.stackrestore.get_or_init(|| unsafe {
            ...
            (
                LLVMAddFunction(self.module, c"llvm.stackrestore.p0".as_ptr(), signature),
                signature,
            )
        })
    }
    ...
    pub unsafe fn expect_i1(&self) -> (LLVMValueRef, LLVMTypeRef) {
        *self.expect_i1.get_or_init(|| {
            ...
            (
                LLVMAddFunction(self.module, c"llvm.expect.i1".as_ptr(), signature),
                signature,
            )
        })
    }
    ...
    pub unsafe fn on_overflow<'env>(&self, builder: &Builder<'env>) -> (LLVMValueRef, LLVMTypeRef) {
        *self.on_overflow.get_or_init(|| {
            ...
            let fn_value = LLVMAddFunction(
                self.module,
                c"$__adept_overflow_panic__".as_ptr(),
                signature,
            );
            let basicblock = LLVMAppendBasicBlock(fn_value, c"".as_ptr());

            let restore = builder.current_block();
    ...
    pub unsafe fn printf(&self) -> (LLVMValueRef, LLVMTypeRef) {
        *self.printf.get_or_init(|| {
            let existing = LLVMGetNamedFunction(self.module, c"printf".as_ptr());
            ...
            let fn_value = if existing.is_null() {
                LLVMAddFunction(self.module, c"printf".as_ptr(), signature)
            } else {
                existing
            };
    ...
    pub unsafe fn abort(&self) -> (LLVMValueRef, LLVMTypeRef) {
        *self.abort.get_or_init(|| {
            let existing = LLVMGetNamedFunction(self.module, c"abort".as_ptr());
            let mut parameter_types = [];
            let is_var_arg = false;
            ...
            let fn_value = if existing.is_null() {
                LLVMAddFunction(self.module, c"abort".as_ptr(), signature)
            } else {
                existing
            };
