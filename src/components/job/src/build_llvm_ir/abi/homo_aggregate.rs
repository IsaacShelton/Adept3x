use super::empty::{IsEmptyRecordOptions, is_empty_record};
use crate::{ir, target_layout::TypeLayoutCache};

#[derive(Copy, Clone, Debug)]
pub struct HomoAggregate<'env> {
    pub base: &'env ir::Type<'env>,
    pub num_members: u32,
}

pub trait HomoDecider<'env> {
    fn is_base_type(
        &self,
        ir_type: &'env ir::Type<'_>,
        type_layout_cache: &TypeLayoutCache<'env>,
    ) -> bool;

    fn is_small_enough(&self, homo_aggregate: &HomoAggregate<'env>) -> bool;
}
...
pub fn is_homo_aggregate<'env>(
    ...
    existing_base: Option<&'env ir::Type<'env>>,
    type_layout_cache: &TypeLayoutCache<'env>,
...
fn is_homo_aggregate_record<'env>(
    decider: &impl HomoDecider<'env>,
