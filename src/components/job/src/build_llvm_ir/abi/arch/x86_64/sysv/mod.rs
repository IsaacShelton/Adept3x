...
use crate::{
    build_llvm_ir::{
        abi::{
            ...
            cxx::Itanium,
            empty::{IsEmptyRecordOptions, is_empty_field},
    ...
    ir,
    target_layout::{ItaniumRecordLayoutBuilder, RecordInfo, TypeLayout, TypeLayoutCache},
};
...
impl SysV {
    pub fn new(os: SysVOs, avx_level: AvxLevel) -> Self {
        Self { os, avx_level }
    }
    ...
    fn classify_return_type<'env>(
        &self,
        ctx: &BackendCtx<'_, 'env>,
        abi: &Itanium<'_, 'env>,
        ir_type: &'env ir::Type<'env>,
    ) -> Result<ABIType, ErrorDiagnostic> {
    ...
    fn classify_argument_type<'env>(
        ...
        is_required: bool,
        is_reg_call: bool,
    ) -> Result<Requirement, ErrorDiagnostic> {
    ...
    fn classify_reg_call_struct_type<'env>(
        &self,
        ctx: &BackendCtx<'_, 'env>,
        abi: &Itanium<'_, 'env>,
        struct_type: &'env ir::Type<'env>,
    ) -> Result<Requirement, ErrorDiagnostic> {
    ...
    fn get_indirect_return_result<'env>(
    ...
    pub fn get_indirect_result<'env>(
        &self,
        ctx: &BackendCtx<'_, 'env>,
        abi: &Itanium<'_, 'env>,
        ir_type: &'env ir::Type<'env>,
    ...
    fn bits_contain_no_user_data_in_record<'env>(
        ctx: &BackendCtx<'_, 'env>,
        start_bit: BitUnits,
        end_bit: BitUnits,
        info: &RecordInfo<'env>,
    ...
    fn bits_contain_no_user_data<'env>(
        ctx: &BackendCtx<'_, 'env>,
        ir_type: &'env ir::Type<'env>,
        start_bit: BitUnits,
        end_bit: BitUnits,
    ...
    fn get_sse_type_at_offset<'env>(
        ctx: &BackendCtx<'_, 'env>,
        llvm_type: LLVMTypeRef,
        llvm_offset: ByteUnits,
        source_type: &'env ir::Type<'env>,
    ...
    fn get_integer_type_at_offset<'env>(
        ctx: &BackendCtx<'_, 'env>,
        llvm_type: LLVMTypeRef,
        llvm_offset: ByteUnits,
        source_type: &'env ir::Type<'env>,
...
fn is_single_element_struct<'env>(
    ctx: &BackendCtx<'_, 'env>,
    ir_type: &'env ir::Type<'env>,
) -> Option<&'env ir::Type<'env>> {
