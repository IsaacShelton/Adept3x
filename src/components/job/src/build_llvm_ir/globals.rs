use super::{backend_type::to_backend_type, ctx::BackendCtx};
...
pub unsafe fn create_globals(ctx: &mut BackendCtx) -> Result<(), ErrorDiagnostic> {
...
pub unsafe fn create_static_variables() -> Result<(), ErrorDiagnostic> {
    Ok(())
}
