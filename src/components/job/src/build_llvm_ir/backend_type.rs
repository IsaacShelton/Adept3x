use super::{
    abi::{
        ...
        abi_type::{ABITypeKind, kinds::TypeExpansion},
    },
...
use crate::{
    build_llvm_ir::abi::abi_type::{Direct, Extend, kinds::get_type_expansion},
    ir,
...
use llvm_sys::{
    core::{
        ...
        LLVMInt1Type, LLVMInt8Type, LLVMInt16Type, LLVMInt32Type, LLVMInt64Type, LLVMPointerType,
...
pub unsafe fn to_backend_type<'a, 'env: 'a>(
    ctx: impl Borrow<ToBackendTypeCtx<'a, 'env>>,
    ir_type: &ir::Type<'env>,
    ...
    Ok(match ir_type {
        ...
        ir::Type::Struct(struct_ref) => to_backend_struct_type(ctx, *struct_ref)?,
        ir::Type::FuncPtr => LLVMPointerType(LLVMInt8Type(), 0),
...
pub unsafe fn to_backend_types<'a, 'env: 'a>(
    ctx: impl Borrow<ToBackendTypeCtx<'a, 'env>>,
    ir_types: impl Iterator<Item = &'env ir::Type<'env>>,
...
pub unsafe fn get_unabi_function_type<'a, 'env: 'a>(
    ctx: impl Borrow<ToBackendTypeCtx<'a, 'env>>,
...
pub unsafe fn get_abi_function_type<'env>(
...
pub unsafe fn get_func_ptr_type<'a, 'env: 'a>(
    ctx: impl Borrow<ToBackendTypeCtx<'a, 'env>>,
    params: &'env [ir::Type<'env>],
...
pub unsafe fn to_backend_mem_type<'a, 'env: 'a>(
    ctx: impl Borrow<ToBackendTypeCtx<'a, 'env>>,
