// Based off of MIT-licensed `threadpool` crate by the Rust Project Developers.
//
// This version relaxes some unnecessary restrictions and adds support for lazily
...
impl Builder {
    ...
    pub fn num_max_total_threads(mut self, max_total_threads: usize) -> Builder {
        assert!(max_total_threads > 0);
        self.max_total_threads = Some(max_total_threads);
        self
    }
    ...
    pub fn build(self) -> ThreadPool {
        ...
        let max_total_threads = self.max_total_threads.unwrap_or(num_threads);
        ...
        let shared_data = Arc::new(ThreadPoolSharedData {
            ...
            max_active_count: AtomicUsize::new(num_threads),
            max_total_threads: AtomicUsize::new(max_total_threads),
...
struct ThreadPoolSharedData {
    ...
    max_active_count: AtomicUsize,
    max_total_threads: AtomicUsize,
...
impl ThreadPool {
    ...
    pub fn new_growable(min_num_threads: usize, max_num_threads: usize) -> Self {
        Builder::new()
            .num_threads(min_num_threads)
            .num_max_total_threads(max_num_threads)
            .build()
    }
    ...
    {
        // Check if needs more threads
        let active = self.shared_data.queued_count.load(Ordering::SeqCst);
        let max_total_threads = self.shared_data.max_total_threads.load(Ordering::SeqCst);
        let max_active = self.shared_data.max_active_count.load(Ordering::SeqCst);

        // Auto-scale
        if active >= max_active && max_active < max_total_threads {
            self.set_max_active_count((max_active * 2).min(max_total_threads));
        }
    ...
    /// assert_eq!(4, pool.max_active_count());
    ///
    /// pool.set_max_active_count(8);
    /// assert_eq!(8, pool.max_active_count());
    /// ```
    pub fn max_active_count(&self) -> usize {
        self.shared_data.max_active_count.load(Ordering::Relaxed)
    }
    ...
    /// pool.set_max_active_count(4);
    ///
    /// assert_eq!(8, pool.active_count());
    /// assert_eq!(2, pool.queued_count());
    /// ```
    pub fn set_max_active_count(&self, num_threads: usize) {
...
impl fmt::Debug for ThreadPool {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("ThreadPool")
            ...
            .field("max_count", &self.max_active_count())
            .finish()
    }
}
    ...
    builder
        .spawn(move || {
            ...
            loop {
                ...
                let max_thread_count_val = shared_data.max_active_count.load(Ordering::Relaxed);
...
mod test {
    ...
    #[test]
    fn test_set_max_active_count_increasing() {
        ...
        pool.set_max_active_count(new_thread_amount);
    ...
    #[test]
    fn test_set_max_active_count_decreasing() {
    ...
    fn test_active_count() {
        ...
        let initialized_count = pool.max_active_count();
    ...
    fn test_shrink() {
        ...
        b0.wait();
        pool.set_max_active_count(TEST_TASKS);
    ...
    fn test_growable() {
        ...
        let pool = Builder::new()
            .thread_name("plant".into())
            .num_threads(8)
            .num_max_total_threads(20)
            .build();
        ...
        sleep(Duration::from_secs(10));
        assert_eq!(pool.max_active_count(), 8);
        ...
        sleep(Duration::from_secs(10));
        assert_eq!(pool.max_active_count(), 20);
